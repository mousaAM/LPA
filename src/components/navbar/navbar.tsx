import { component$, useStore, useContext, useClientEffect$, useSignal } from "@builder.io/qwik";
...
export default component$(() => {
  ...
  return (
    <nav dir={englishUsed.value ? "ltr" : "rtl"}
      ...
      <div>
        <img id="navbar-btn" class="block md:hidden  mx-4 px-2 py-1" onClick$={() => state.visibility = !state.visibility}
        ...
        <MUIMenu anchorEl={anchorEl.value} open={state.visibility} onClose$={() => {state.visibility = !state.visibility}}>
          {englishUsed.value 
          ? state.navbarElements.en.map((value, index) =>{
            return <Link 
                key={index}
          ...
          : state.navbarElements.ar.map((value, index) => {
            return <Link 
                ...
                class="block text-md md:inline rounded hover:text-sky-100 mx-4 my-4 md:my-0 px-2 py-1"
                href={state.links[index]}>
              <MUIMenuItem className="font-sans">
                {value}
              </MUIMenuItem>
            </Link>
          })}
